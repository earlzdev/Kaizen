...
async def cheapest_flights(
    origin: str, destination: str, depart_date: str, days_window: int = 5, language: str = "en"
) -> str:
    ...
    o = await _flights.resolve_city(origin, language)
    d = await _flights.resolve_city(destination, language)
    if o is None:
    ...
    try:
        offers = await _flights.cheapest(
            o[0], d[0], depart, days_window=max(1, min(days_window, 14)), language=language
        )
...
TOOL = ToolDef(
    ...
    description=(
        ...
        "text (resolved to airports); depart_date is YYYY-MM-DD. Pass `language` "
        "matching the language you're currently replying in — it also selects the "
        "currency (USD for en, RUB for ru)."
    ),
    input_schema={
        "type": "object",
        "properties": {
            ...
            "language": {"type": "string", "enum": ["en", "ru"], "description": "Output language + currency (default en)"},
        },
